...
function MissionMenu:new(backgroundOverlay)
    ...
    local xmlFile = loadXMLFile("missions.xml", "data/missions" .. g_languageSuffix .. ".xml");
    ...
    --backPlayButtonSpacing+3*buttonHeight + 2*upDownButtonSpacing + 5*imageSpacing + 4*imageSize = 1
    ...
    repeat
        ...
        if mission.id ~= nil then
            ...
            mission.bronze = getXMLFloat(xmlFile, baseXMLName .. ".bronze")*1000;
            mission.silver = getXMLFloat(xmlFile, baseXMLName .. ".silver")*1000;
            mission.gold = getXMLFloat(xmlFile, baseXMLName .. ".gold")*1000;
            if g_isDemo then
    ...
    for missionId in string.gmatch(finished, "%w+") do
        ...
        --print(g_finishedMissions[missionId+0], " ", g_finishedMissionsRecord[missionId+0]);
        count = count + 1;
    end;

    instance:addButton(OverlayButton:new(Overlay:new("up_button", "data/menu/up_button.png", 0.5-0.5*instance.buttonWidth, 1-instance.upDownButtonSpacing-instance.buttonHeight, instance.buttonWidth, instance.buttonHeight), OnMissionMenuScrollUp));
    instance:addButton(OverlayButton:new(Overlay:new("down_button", "data/menu/down_button.png", 0.5-0.5*instance.buttonWidth, instance.upDownButtonSpacing+instance.backPlayButtonSpacing+instance.buttonHeight, instance.buttonWidth, instance.buttonHeight), OnMissionMenuScrollDown));

    instance:addButton(OverlayButton:new(Overlay:new("back_button", "data/menu/back_button".. g_languageSuffix .. ".png", 0.65, instance.backPlayButtonSpacing, instance.buttonWidth, instance.buttonHeight), OnMissionMenuBack));
    instance:addButton(OverlayButton:new(Overlay:new("play_button", "data/menu/ingame_play_button".. g_languageSuffix .. ".png", 0.65+instance.buttonWidth+instance.backPlayButtonSpacing, instance.backPlayButtonSpacing, instance.buttonWidth, instance.buttonHeight), OnMissionMenuPlay));
    ...
    table.insert(instance.overlays, Overlay:new("background_overlay", "data/menu/missionmenu_background.png", instance.spacingLeft*0.5, instance.selectedPositionBase, 1-instance.spacingLeft, 4*(instance.imageSpacing+instance.imageSize)));
    instance.selectedOverlay = Overlay:new("selected_overlay", "data/menu/missionmenu_selected.png", 0, 0, 1-instance.spacingLeft, instance.imageSpacing+instance.imageSize);
    table.insert(instance.overlays, instance.selectedOverlay);

    instance.emptyMedalOverlay = Overlay:new("emptyMedalOverlay", "data/missions/empty_medal.png", 0, 0, instance.imageSize*0.75, instance.imageSize);
    instance.bronzeMedalOverlay = Overlay:new("bronzeMedalOverlay", "data/missions/bronze_medal.png", 0, 0, instance.imageSize*0.75, instance.imageSize);
    instance.silverMedalOverlay = Overlay:new("silverMedalOverlay", "data/missions/silver_medal.png", 0, 0, instance.imageSize*0.75, instance.imageSize);
    instance.goldMedalOverlay = Overlay:new("goldMedalOverlay", "data/missions/gold_medal.png", 0, 0, instance.imageSize*0.75, instance.imageSize);

    if g_isDemo then
        instance.demoLockedOverlay = Overlay:new("demoLockedOverlay", "data/menu/demo_locked.png", 0, 0, instance.imageSize*0.75, instance.imageSize);
    end;
...
function MissionMenu:render()
    ...
    for i=self.startIndex, endIndex do
        ...
        overlay:render();
        if g_isDemo then
            if mission.id ~= 11 and mission.id ~= 13 then
        ...
        setTextBold(true);
        renderText(textLeft, 1-(self.spacingTop+(self.imageSize+self.imageSpacing)*(i-self.startIndex) + self.textSizeTitle), self.textSizeTitle, mission.name);
        setTextBold(false);
        ...
        if mission.id ~= nil and g_finishedMissions[mission.id] ~= nil and g_finishedMissionsRecord[mission.id] ~= nil then
            ...
            local timeMinutesF = g_finishedMissionsRecord[mission.id]/(1000*60);
            ...
            --print("recordFloor ", recordFloor, " mission.bronze" , mission.bronze);
            --print("mission.silver ", mission.silver, " mission.gold" , mission.gold);
            ...
            local recordStr = string.format(g_i18n:getText("Record") .. ": %02d:%02d %s", timeMinutes, timeSeconds, medalStr);
            setTextBold(true);
...
function MissionMenu:startSelectedMission()
    ...
    stats.farmSiloWheatAmount = 0;
    stats.storedWheatFarmSilo = 0;
    stats.soldWheatPortSilo = 0;
    stats.revenue = 0;
    stats.expenses = 0;
    stats.playTime = 0;
    stats.money = 1000;
    ...
    gameMenuSystem.medalsDisplay:setTimes(mission.bronze, mission.silver, mission.gold);

...
function VehicleCamera:new(cameraNode, isRotatable, rotateNode, limit, rotMinX, rotMaxX, transMin, transMax, customMt)
    ...
    local trans1OverLength = 1.0/Utils.vector3Length(instance.origTransX, instance.origTransY, instance.origTransZ);
...
function VehicleCamera:update(dt)
    if self.isActivated then
        if self.isRotatable then
            ...
            self.rotX = self.rotX + rotSpeed*getInputAxis(Input.AXIS_W);
            self.rotY = self.rotY + rotSpeed*getInputAxis(Input.AXIS_Z);
        end;
    end;
